...
-- BRAIN
function Brain.save(filename)
	torch.save(filename, Brain.net)
end

function Brain.init(num_states, num_actions, previous_network)
   Brain.episodes = 0
   Brain.totalReward = 0
   Brain.actionTime = 1
   ...
   Brain.random_action_distribution = {};
    if(table.length(Brain.random_action_distribution) > 0) then
      ...
      if(math.abs(s - 1.0) > 0.0001) then
   	...
   	-- define architecture 
	previous_network = previous_network or ''
	   ...
	   Brain.net:add(nn.Linear(Brain.hidden_nodes, Brain.net_outputs))
		...
		print('Loading network from ' .. previous_network)
		os.execute("sleep " .. tonumber(2))
		Brain.net = torch.load(previous_network)
	end
